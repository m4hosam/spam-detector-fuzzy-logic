...
# albert_spam_probability['not_spam'] = fuzz.trapmf(albert_spam_probability.universe, [0, 0, 51])
# albert_spam_probability['maybe_spam'] = fuzz.trapmf(albert_spam_probability.universe, [0, 51, 101])
# albert_spam_probability['definite_spam'] = fuzz.trapmf(albert_spam_probability.universe, [51, 101, 101])
#
# # Creating memberships for user model - Input Variable
# tfidf_score['not_spam'] = fuzz.trapmf(tfidf_score.universe, [0, 0, 51])
# tfidf_score['maybe_spam'] = fuzz.trapmf(tfidf_score.universe, [0, 51, 101])
# tfidf_score['definite_spam'] = fuzz.trapmf(tfidf_score.universe, [51, 101, 101])
#
# # Creating memberships for tweet model - Output Variable
# predict_spam['not_spam'] = fuzz.trapmf(predict_spam.universe, [0, 0, 101])
# predict_spam['spam'] = fuzz.trapmf(predict_spam.universe, [0, 101, 101])
#
...
# rule3 = ctrl.Rule(albert_spam_probability['not_spam'] & tfidf_score['definite_spam'], predict_spam['spam'])
#
...
#
# rule7 = ctrl.Rule(albert_spam_probability['definite_spam'] & tfidf_score['not_spam'], predict_spam['spam'])
...
#
# spam_percentage.input['albert_spam_probability'] = 50
# spam_percentage.input['tfidf_score'] = 50
#
# # Crunch the numbers
...
class SpamFuzzyController:
    ...
    def fuzzy_predict(self, tweet_proba, tfidf_score_proba):
        spam_percentage = self._spam_percentage
        spam_percentage.input['albert_spam_probability'] = tweet_proba
        spam_percentage.input['tfidf_score'] = tfidf_score_proba

        # Crunch the numbers
        spam_percentage.compute()

        print(" albert_spam_probability - {0} | tfidf_score = {1} | Fuzzy Prediction = {2}".format(tweet_proba, tfidf_score_proba,
